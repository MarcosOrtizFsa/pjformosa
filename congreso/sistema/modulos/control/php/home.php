...
$t->set_var("titulo_modulo","Usuarios");
...
$where="";			
if ( isset($system_07_ver)!="" or isset($system_07_priv)!="" or isset($variable_buscar)!="" or isset($system_estado)!="")
{
		if ( $system_estado!='' )
		{
		...
		$where.=" and system_04_perfil.rela_system_03=system_03_usuarios.id_system_03 and system_07_privilegios.id_system_07=system_03_usuarios.rela_system_07 ";
...
{
$and  = " and  system_03_usuarios.system_03_modo IN ('0','1','2','3') ";								
}
else
if ($sesion_system_07=='2')
{
$and  = " and  system_03_usuarios.system_03_modo IN  ('1','2','3') ";	
}
else
{
$and  = " and  system_03_usuarios.system_03_modo IN ('3') ";	
}
//  	0=root, 1=gerente, 2=admin, 3=usuario 
//echo $where_control;
$tSQL = $mysqli -> consulta_SQL("Select COUNT(*) as total_filas from system_04_perfil, system_03_usuarios, system_07_privilegios where 
...
if ($row == TRUE)
{
	...
	{	
		...
		//$t->set_var("system_07_descripcion",$row[$i]['system_07_descripcion']);
		$t->set_var("system_07_nombre",		$row[$i]['system_07_nombre']);
		$t->set_var("system_03_fecha",		$row[$i]['system_03_fecha']);
		...
		// elimiar
		if ( optener_permisos('B',$id_system_01,$sesion_system_03,$mysqli) == '1' || $candado=='on' )
		{
			$url="'modulos/control/php/_interfaz.php'";
...
else 						
{
	...
	// SERVICIOS POPUP
	if ($sesion_system_07=='1' or $sesion_system_07=='2')
	{
	$url="\"modulos/especialistas/php/home.php\"";
	$id="\"popup\"";
	$vars="\"id_system_01=6\"";	
	$t->set_var("funcion_servicios","cargar_post($url,$id,$vars)");
	$t->set_var("ver_niveles","alt");
	}
	else
	{
	$t->set_var("funcion_servicios","");
	$t->set_var("ver_niveles","hide");
	}
...
$pagExc='<a href="modulos/control/php/informe_csv.php?name_archivo=lista_registrados" target="news">x</a>';
